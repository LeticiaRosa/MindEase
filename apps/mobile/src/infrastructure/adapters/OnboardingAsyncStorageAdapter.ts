...
export class OnboardingAsyncStorageAdapter implements IOnboardingStateRepository {
  async load(): Promise<OnboardingState> {
    try {
      const raw = await AsyncStorage.getItem(ONBOARDING_STATE_STORAGE_KEY);
  ...
  async clear(): Promise<void> {
    await AsyncStorage.removeItem(ONBOARDING_STATE_STORAGE_KEY);
  }
}
